...
if (isset($_POST['update'])) {
    ...
    $not_id_to_update = $_POST['not_id']; // Formdan alınan not id'si

    // ID'ye göre güncelleme sorgusu
    $updateSql = "UPDATE notlar SET vize = ?, final = ?, butunleme = ?, dersadi = ? WHERE id = ?";
    $updateParams = array($vize, $final, $butunleme, $dersadi, $not_id_to_update);
...
if (isset($_POST['delete_not'])) {
    $not_id_to_delete = $_POST['not_id']; // Silinecek notun id'sini alıyoruz
    $deleteNotSql = "DELETE FROM notlar WHERE id = ?";
    $deleteNotParams = array($not_id_to_delete);
    $deleteNotStmt = sqlsrv_query($baglan, $deleteNotSql, $deleteNotParams);

    if ($deleteNotStmt === false) {
        die(print_r(sqlsrv_errors(), true));
    } else {
        echo "<div class='alert alert-success' role='alert'>Not başarıyla silindi.</div>";
        
        // Silindikten sonra notları tekrar sorgula
        $sqlNotlar = "SELECT * FROM notlar WHERE ogr_no = ?";
        $paramsNotlar = array($ogr_no); // Öğrenci numarasını kullan
        $stmtNotlar = sqlsrv_query($baglan, $sqlNotlar, $paramsNotlar);

        // Notları sıfırla
        $notlar = []; 

        // Veritabanından geri kalan notları al
        while ($row = sqlsrv_fetch_array($stmtNotlar, SQLSRV_FETCH_ASSOC)) {
            $notlar[] = $row; // Notları diziye ekle
        }

        // Eğer başka not yoksa "Bu öğrenciye ait başka not bulunmamaktadır" mesajı göster
        if (empty($notlar)) {
            echo "<div class='alert alert-warning' role='alert'>Bu öğrenciye ait başka not bulunmamaktadır.</div>";
        }
    }
}
...
<body>
    <div class="container mt-5">
        ...
        <?php if (!empty($ogr_no)): ?>
            <h3><?php echo htmlspecialchars($ogr_no); ?> Numaralı Öğrencinin Notları</h3>
            <?php if (!empty($notlar)): ?>
                <table class="table table-striped">
                    <thead>
                        <tr>
    ...
    <?php foreach ($notlar as $not): ?>
        <tr>
            ...
            <form method="POST" action="">
                <input type="hidden" name="not_id" value="<?php echo $not['id']; ?>" />  <!-- Her not için id -->
                <td>
            ...
            <!-- Silme formu -->
            <form method="POST" action="">
                <input type="hidden" name="not_id" value="<?php echo $not['id']; ?>" />  <!-- Silinecek not için id -->
                <input type="hidden" name="ogr_no_to_delete" value="<?php echo htmlspecialchars($not['ogr_no']); ?>" />
                <td>
